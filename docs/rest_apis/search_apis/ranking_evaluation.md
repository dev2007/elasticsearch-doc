...
排序评估 API 允许你评估一组典型搜索查询中排名搜索结果的质量。给定这组查询和手动分级的文档列表，`_rank_eval` 端点计算并返回典型的信息检索指标，如*平均倒数排名、精度或折扣累积增益*。
...
排序评估 API 提供了一种方便的方法，可以在排名评估请求中使用此信息来计算不同的搜索评估指标。这为你提供了总体搜索质量的第一个估计，以及在微调应用程序中查询生成的各个方面时要进行优化的度量。
...
你也可使用[存储的模板](/search_your_data/search_templates#创建一个搜索模板)。
