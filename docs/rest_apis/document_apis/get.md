...
- 开启 Elasticsearch 安全功能后，你必须拥有目标索引或索引别名的 `read` [索引权限](/secure_the_elastic_statck/user_authorization/security_privileges#索引权限)。

## 描述

使用 GET 可从特定索引中检索文档及其源字段或存储字段。使用 HEAD 可验证文档是否存在。你可以使用 `_source` 资源检索文档源或验证其是否存在。
...
默认情况下，除非使用了 `stored_fields` 参数或禁用了 `_source` 字段，否则 get 操作将返回 `_source` 字段的内容。你可以使用 `_source` 参数关闭 `_source` 检索：
...
你可以使用源过滤参数来控制 `_source` 的哪些部分会被返回：

```bash
GET my-index-000001/_source/1/?_source_includes=*.id&_source_excludes=entities
```

你可以使用 HEAD 和 `_source` 端点来有效地测试文档 `_source` 是否存在。如果在[映射](/mapping/metadata_fields/_source_field)中禁用了文档源，则文档源不可用。
