...
- 如果 Elasticsearch 安全特性启用，你必须有 `read` [索引权限](/secure_the_elastic_statck/user_authorization/security_privileges#索引权限)来向目标数据流、索引或别名使用此 API。对于跨集群搜索，参阅[配置跨集群搜索权限](/set_up_elasticsearch/remote_clusters/configure_roles_and_users_for_remote_clusters#配置跨集群搜索权限)。

  要在[时间点（PIT）](/rest_apis/search_apis/point_in_time)中搜索别名，必须对别名数据流或索引有 `read` 索引权限。

## 描述

允许你执行搜索查询并返回与查询匹配的搜索命中。你可以使用 [`q` 查询字符串参数](/rest_apis/search_apis/search#查询参数)或[请求体](https://www.elastic.co/guide/en/elasticsearch/reference/current/search-request-body.html#search-request-body)提供搜索查询。
...
- `allow_partial_search_results`
  ...
  （可选，布尔值）如果为 `true`，则在存在分片请求超时或[分片失败](/document_apis/reading_and_writing_documents#分片失败)时返回部分结果。如果为 `false`，则返回没有部分结果的错误。默认为 `true`。
...
- `from`
  ...
  默认情况下，使用 `from` 和 `size` 参数不能翻阅超过 10,000 次命中。要翻阅更多命中，请使用 [`search_after`](/search_your_data/paginate_search_results#搜寻) 参数。
...
- `preference`
  ...
  （可选，字符串）用于搜索的节点和分片。默认情况下，Elasticsearch 使用[自适应副本选择](/search_your_data/search_shard_routing#自适应副本选择)从符合条件的节点和分片中进行选择，以考虑[分配感知](/set_up_elasticsearch/configuring_elasticsearchcluster_level_shard_allocation_and_routing_settings#分片分配感知)。
...
- `size`
  ...
  默认情况下，使用 `from` 和 `size` 参数不能翻页浏览超过 10,000 次点击。要浏览更多点击，请使用 [`search_after`](/search_your_data/paginate_search_results#搜寻)参数。
...
- `timeout`
  ...
  （可选，[时间单位](/rest_apis/api_convention/common_options#时间单位)）指定等待每个分片响应的时间段。如果在超时过期之前没有收到响应，则请求失败并返回错误。默认为无超时。
...
- `indices_boost`
    ...
    （可选，对象数组）提高指定索引中文档的 [`_score`](/query_dsl/query_and_filter_context#相关性得分)。
  ...
  - `min_score`
    ...
    （可选，浮点数）匹配文档的最低 [`_score`](/query_dsl/query_and_filter_context#相关性得分)。搜索结果中不包括 `_score` 较低的文档。
  ...
  - `pit`
    ...
    - `pit` 属性
      ...
      - `keep_alive`
        ...
        （可选，[时间值](/rest_apis/api_convention/common_options#时间单位)）用于延长 PIT 生存周期。
...
- `_scroll_id`
  ...
  （字符串）搜索及其[搜索上下文](/search_your_data/paginate_search_results#保持搜索上下文存活)的标识符。

  你可以将此滚动 ID 与[滚动 API](/rest_apis/search_apis/scroll) 一起使用，以检索请求的下一批搜索结果。参阅[滚动搜索结果](/search_your_data/paginate_search_results#滚动搜索结果)。
...
- `_shards`
  ...
  - `_shards` 属性
    ...
    - `failed`
      ...
      （整数）无法执行请求的分片数。请注意，未分配的分片将被视为既不成功也不失败。因此，`failed+successful` 少于 `total` 表示未分配部分分片。
