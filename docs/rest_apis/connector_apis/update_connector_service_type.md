...
- `service_type` 必须是连接器框架中定义的有效类型。
  - 更改已配置连接器的 `service_type` 时，你还需要重置其配置以确保兼容性。
...
> [原文链接](https://www.elastic.co/guide/en/elasticsearch/reference/8.18/update-connector-service-type-api.html)
