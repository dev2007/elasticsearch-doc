...
由于这种动态的逐段重写，干净的树结构变得扭曲，不再遵循显示一个查询如何重写到下一个查询的干净“沿袭”。目前，我们所能做的就是道歉，如果太令人困惑，建议你折叠查询子项的详细信息。幸运的是，所有的计时统计信息都是正确的，只是响应中的物理布局不正确，因此，如果你发现细节太复杂而无法解释，只分析顶级 MultiTermQuery 并忽略其子项就足够了。
